...
Sleep policy: idle currently uses `SLEEP_NORMAL` (not deep sleep) so J-Link can halt the core for debugging — see the comment in `am_freertos_sleep()` in [src/rtos.c:107](src/rtos.c#L107). Switch to `SLEEP_DEEP` for production.
...
#### [src/ble_freertos_fit.c](src/ble_freertos_fit.c)
...
#### [src/ble_freertos_fit.h](src/ble_freertos_fit.h)
...
#### [src/rtos.c](src/rtos.c) / [src/rtos.h](src/rtos.h)
FreeRTOS glue:
...
#### [src/FreeRTOSConfig.h](src/FreeRTOSConfig.h)
Standard FreeRTOS config. Notable: ticks at 1 kHz, heap is `heap_4`, stack overflow detection enabled.

### BLE / radio

#### [src/radio_task.c](src/radio_task.c) / [src/radio_task.h](src/radio_task.h)
...
The actual GATT service list and advertising payload lives outside this file in `third_party/cordio/.../fit/fit_main.c` and is patched in via [patches/fit_main.patch](patches/fit_main.patch). See section 7 below.

#### [src/hci_apollo_config.h](src/hci_apollo_config.h)
Pin assignments for the BLE radio's HCI UART. Don't edit unless you change boards.

#### [src/ble_json_svc.c](src/ble_json_svc.c)
...
#### [src/ble_opus_stream_svc.c](src/ble_opus_stream_svc.c) / [src/ble_opus_stream_svc.h](src/ble_opus_stream_svc.h)
Custom **Opus audio stream service** — UUID `...90BB`, value char `...90BC`, notify-only. This is the bulk of the BLE complexity.

Implements the **windowed stop-and-wait flow protocol** documented in [CLAUDE.md](CLAUDE.md): packet header (flags / version / packet_index / total_packets / total_len), `START` / `END` / `WINDOW_END` flags, ACK / Negative Acknowledgment (NACK) handling, selective retransmit, ACK-timeout auto-resend.
...
#### [src/fake_opus_audio.c](src/fake_opus_audio.c) / [src/fake_opus_audio.h](src/fake_opus_audio.h)
...
#### [src/mic_task.c](src/mic_task.c) — the heart of the firmware
...
#### [src/mic_task.h](src/mic_task.h)
Public surface: `MicTask()` task entry point and `MicTaskGetOpusData()` accessor.

#### [src/am_resources.c](src/am_resources.c)
HAL resource hooks. Should not need editing.

### Keyword spotting

#### [src/kws_inference.cc](src/kws_inference.cc) / [src/kws_inference.h](src/kws_inference.h)
C++ glue around TFLM:
...
#### [src/kws/kws_model_settings.h](src/kws/kws_model_settings.h)
...
#### [src/kws/kws_model_data.h](src/kws/kws_model_data.h)
...
#### [gcc/linker_script.ld](gcc/linker_script.ld) / [gcc/startup_gcc.c](gcc/startup_gcc.c)
...
#### [gcc/flash.jlink](gcc/flash.jlink)
...
#### [patches/fit_main.patch](patches/fit_main.patch)
...
#### [patches/hci_vs_cooper.patch](patches/hci_vs_cooper.patch)
Smaller patch to the EM9304 HCI driver (vendor-specific commands).

#### [scripts/apply_vendor_patches.sh](scripts/apply_vendor_patches.sh)
...
1. Write the service file (model after [src/ble_json_svc.c](src/ble_json_svc.c)).
2. Add a `*SvcAdd()` call in the SDK's `fit_main.c` — update [patches/fit_main.patch](patches/fit_main.patch).
3. Add a CCCD entry to `fitCccSet[]` in `fit_main.c` (also via the patch).
4. Bump WSF buffer pool 6 size in [src/radio_task.c](src/radio_task.c) if the GATT DB hash buffer overflows.
5. Re-run `make vendor-patch-apply` and `make -C gcc`.

If you need to **change the audio config** (sample rate, recording length, gain): edit the `#define`s at the top of [src/mic_task.c](src/mic_task.c). Be careful — `REC_PCM_RATE` is fixed at 16 kHz by the Opus encoder library and cannot change without rebuilding libopus with different config. `REC_SECONDS` is the easiest knob.

If you need to **swap the KWS model**: replace [src/kws/kws_model_data.h](src/kws/kws_model_data.h), update `kws_label_names[]` and `KWS_TRIGGER_IDX` in [src/kws_inference.cc](src/kws_inference.cc), and bump `KWS_NUM_FRAMES` / `KWS_NUM_COEFFS` / `KWS_NUM_CLASSES` if the input/output shape changed. The op list registered via `s_resolver.AddXxx()` must match the ops your model uses — TFLM will fail at `AllocateTensors()` if any op is missing.
...
|----------------|-----------|
| Add / change a BLE service | [src/ble_json_svc.c](src/ble_json_svc.c) + [patches/fit_main.patch](patches/fit_main.patch) |
| Tweak record length / mic gain | top of [src/mic_task.c](src/mic_task.c) |
| Adjust the streaming flow protocol | [src/ble_opus_stream_svc.c](src/ble_opus_stream_svc.c) |
| Swap or retrain the KWS model | [src/kws/kws_model_data.h](src/kws/kws_model_data.h), [src/kws_inference.cc](src/kws_inference.cc) |
| Add a FreeRTOS task | [src/rtos.c](src/rtos.c) `setup_task()` |
| Change peripheral pin / clock config | [src/hci_apollo_config.h](src/hci_apollo_config.h), top of [src/mic_task.c](src/mic_task.c) |
| Update SDK include paths / link libs | [gcc/Makefile](gcc/Makefile) |
| Change advertising name | [patches/fit_main.patch](patches/fit_main.patch) (`fitScanDataDisc[]`) |