...
#define WW_TENSOR_ARENA_SIZE (96 * 1024)

__attribute__((section(".sram_bss"))) alignas(16) static uint8_t s_arena[WW_TENSOR_ARENA_SIZE];
// back to:
//AM_SHARED_RW alignas(16) static uint8_t s_arena[WW_TENSOR_ARENA_SIZE];

static tflite::MicroMutableOpResolver<WW_NUM_OPS> s_resolver;
...
// Order must match class_names from training: ['nothing','iam','note','lifeiko']
const char *ww_label_names[WW_NUM_CLASSES] = {
    "nothing", "iam", "note", "lifeiko"
};
...
void ww_init(void) {
    ...
    am_util_stdio_printf("[chewallow] init: GetModel addr=%p first8=%02x%02x%02x%02x%02x%02x%02x%02x\r\n",
                         g_ww_model_data,
                         g_ww_model_data[0], g_ww_model_data[1],
                         g_ww_model_data[2], g_ww_model_data[3],
                         g_ww_model_data[4], g_ww_model_data[5],
                         g_ww_model_data[6], g_ww_model_data[7]);
    s_model = tflite::GetModel(g_ww_model_data);
    ...
    s_interpreter = new(s_interp_buf) tflite::MicroInterpreter(
        s_model, s_resolver, s_arena, WW_TENSOR_ARENA_SIZE);
    ...
    am_util_stdio_printf(" (WW_INPUT_SIZE compile-time=%d)\r\n", WW_INPUT_SIZE);
    ...
    am_util_stdio_printf(" (WW_NUM_CLASSES compile-time=%d)\r\n", WW_NUM_CLASSES);

    am_util_stdio_printf("[chewallow] Ready. Arena used: %d / %d bytes\n",
                         (int)s_interpreter->arena_used_bytes(), WW_TENSOR_ARENA_SIZE);
}

// Shared implementation for ww_run / ww_run_dummy_timing.
...
static int ww_run_impl(const float *mfcc_features, float *p_ww_out) {
    ...
    // Write every class probability. The device applies a different
    // threshold per class -- and a different one again depending on
    // context -- so a single scalar is not enough.
    if (p_ww_out) {
        for (int i = 0; i < out_count && i < WW_NUM_CLASSES; i++) {
            p_ww_out[i] = ((float)s_output->data.int8[i] - (float)out_zp) * out_scale;
        }
    }
    return best_idx;
}
...
int ww_run(const float *mfcc_features, float *p_ww_out) {
    return ww_run_impl(mfcc_features, p_ww_out);
}

int ww_run_dummy_timing(const float *dummy_features, float *p_ww_out) {
    return ww_run_impl(dummy_features, p_ww_out);
}
